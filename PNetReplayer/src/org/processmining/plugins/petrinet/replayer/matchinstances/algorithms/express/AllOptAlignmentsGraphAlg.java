...
@KeepInProMCache
public class AllOptAlignmentsGraphAlg extends AbstractAllOptAlignmentsGraphAlg<PDelegate, DijkstraTail> {
	public String toString() {
		return "Graph-based state space replay to obtain all optimal alignments.";
	}

	public String getHTMLInfo() {
		return "<html>Returns representative alignments (without sorting of states) using graph-based state space. <br/>"
				+ "Assuming that the model does not allow loop/infinite firing sequences of cost 0. <br/>"
				+ "Reordering of sync moves is taken into account (hence all optimal alignments are obtained)<br/>"
	 ...
	 * @return
	 */
	//	protected int computeThresholdCost(PRecord r, PDelegate d, Trace filteredTrace){
	//		int cost = 0;
	...
	/**
	 * Method to be overriden if the thread needs to be changed. This return a
	 * thread for which ordering of movements is applied
	 * 
	 * @param aStar
	 * @param initial
	 * @param trace
	...
	protected AllOptAlignmentsGraphThread<PHead, DijkstraTail> getThread(
			...
			throws AStarException {
		return new AllOptAlignmentsGraphThread.MemoryEfficient<PHead, DijkstraTail>(aStar, initial, trace,
				maxNumOfStates);
