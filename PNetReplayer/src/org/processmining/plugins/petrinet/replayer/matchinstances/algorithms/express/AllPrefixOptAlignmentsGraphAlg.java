...
/**
 * @author aadrians Mar 2, 2013
 * 
 */
//@PNReplayMultipleAlignmentAlgorithm
public class AllPrefixOptAlignmentsGraphAlg extends AbstractAllOptAlignmentsGraphAlg<PDelegate, DijkstraTail> {
	public String toString() {
		return "Graph-based state space replay to obtain representatives of prefix optimal alignments";
	}

	@Override
	public String getHTMLInfo() {
		return "<html>Returns representatives of prefix optimal alignments using graph-based state space. <br/>" +
		...
		for (int i = 0; i < log.size(); i++) {
			...
			result.add(pool.submit(new Callable<MatchInstancesGraphRes>() {
				...
				public MatchInstancesGraphRes call() throws Exception {
					...
					if (result.reliable) {
						// get real bound
						int realBound = (int) ((Math.floor(record.getCostSoFar() / delegate.getDelta()) + 1) * delegate
								.getDelta()) - 1;
						if (realBound < record.getCostSoFar()) {
							realBound = record.getCostSoFar();
						}

						record = (PRecord) thread.getOptimalRecord(c, realBound,-1.0);
	...
	protected AllOptAlignmentsGraphThread<PHead, DijkstraTail> getThread(
		...
		return new AllOptAlignmentsGraphThread.MemoryEfficient<PHead, DijkstraTail>(aStar, initial, trace, maxNumOfStates);
	}
