...
public class PILPTailCompressor extends AbstractCompressor<PILPTail> implements TailInflater<PILPTail> {
	...
	public PILPTail inflate(InputStream stream) throws IOException {
		...
		return new PILPTail(est, vars, exact);
	}
	...
	public <H extends Head> int inflateEstimate(StorageAwareDelegate<H, PILPTail> delegate, H head, InputStream stream)
			throws IOException {
		skipHead(stream);
		stream.skip(1);
