...
public class PHeadCompressor<T extends Tail> extends AbstractCompressor<PHead> implements
		EqualOperation<State<PHead, T>>, HashOperation<State<PHead, T>>, HeadDeflater<PHead> {
	...
	public PHead inflate(InputStream stream) throws IOException {
		...
		assert PHead.PROVIDER.hash(marking, parikh) == hashCode;
		return new PHead(marking, parikh, hashCode);//, hashCode, bits);
	}
	...
	protected boolean equalsInflating(State<PHead, T> vector, CompressedStore<State<PHead, T>> store, long l)
			throws IOException {
	...
	public void skip(PHead head, SkippableOutputStream out) throws IOException {
		// skip hashCode
		int toSkip = 4;
		// skip marking
		toSkip += bytesPlaces;
