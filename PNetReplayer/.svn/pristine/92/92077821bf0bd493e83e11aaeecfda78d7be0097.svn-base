...
public class PetrinetReplayerWithILP extends AbstractPetrinetReplayer<PILPTail, PILPDelegate> {
	...
	public PetrinetReplayerWithILP() {
		this(true, true);
	}

	public PetrinetReplayerWithILP(boolean useInt, boolean useFastLowerBound) {
		this.useInt = useInt;
	...
	protected PILPDelegate getDelegate(PetrinetGraph net, XLog log, XEventClasses classes, TransEvClassMapping mapping,
		...
		if (net instanceof ResetInhibitorNet) {
			...
			return new PILPDelegate((ResetNet) net, log, classes, mapping, mapTrans2Cost, mapEvClass2Cost, mapSync2Cost,
			...
			return new PILPDelegate((Petrinet) net, log, classes, mapping, mapTrans2Cost, mapEvClass2Cost, mapSync2Cost,
					delta, threads, useInt, useFastLowerBound, finalMarkings);
		}
