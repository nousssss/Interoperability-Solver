...
public class PILPTail implements FastLowerBoundTail {
	...
	public PILPTail(AbstractPILPDelegate<?> delegate, PHead h, int minCost) {
	...
	protected PILPTail(int estimate, short[] variables, boolean isExact) {
		if (estimate < 0) {
			this.estimate = 0;
		} else {
	...
	public PILPTail getNextTail(Delegate<? extends Head, ? extends Tail> d, Head newHead, int modelMove, int logMove,
		...
		if (modelMove == AStarThread.NOMOVE) {
			...
			if (variables[var] >= 1) {
				...
				newVars[var] -= 1;
				return new PILPTail(newEst, newVars, true);
			}
		...
		return new PILPTail(delegate, (PHead) newHead, newEst);//Math.max(newEst, ((PHead) newHead).getParikhVector().getNumElts()));
	}
	...
	public boolean canComplete() {
		return estimate != AbstractPILPDelegate.INFEASIBLE_INT;
	}
	...
	public void computeEstimate(Delegate<? extends Head, ? extends Tail> d, Head head, int lastEstimate) {
		...
		PHead h = (PHead) head;
		AbstractPILPDelegate<?> delegate = (AbstractPILPDelegate<?>) d;
		...
		} else if (res.getResult() == AbstractPILPDelegate.INFEASIBLE) {
			solvedEstimate = AbstractPILPDelegate.INFEASIBLE_INT;
		} else {
		...
		if (solvedEstimate == AbstractPILPDelegate.INFEASIBLE_INT || solvedEstimate > lastEstimate) {
