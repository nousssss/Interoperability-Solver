...
public class UmaPromUtil {
  ...
  /**
	...
	*         converted to ProM format, and a mapping from <code>net</code> nodes to the
	*         converted net nodes, the converted net has the given <code>netLabel</code>
	*/
  ...
  public static Marking toPromFormat(Collection<hub.top.petrinet.Place> marking, Map<hub.top.petrinet.Node, PetrinetNode> nodeMap) {
	Marking promMarking = new Marking();
	for (hub.top.petrinet.Place _p : marking) promMarking.add((Place)nodeMap.get(_p));
	return promMarking;
  }
