...
public class Uma_RepairModel_UI extends ProMPropertiesPanel {
	...
	private javax.swing.JCheckBox repairFinalMarkingBox;
	...
	public Uma_RepairModel_UI(RepairConfiguration config) {
		...
		remove_keepIfMoreThan = addTextField("keep if occurs at least", Integer.toString(config.remove_keepIfAtLeast), 1, 400);
		...
		repairFinalMarkingBox = addCheckBox("repair final marking", config.repairFinalMarking, 0, 250);
	}
	...
	private void getChosenParameters(RepairConfiguration config) {
		...
		config.repairFinalMarking = repairFinalMarkingBox.isSelected();
	}
	...
	protected Object[] userCancel(PluginContext context) {
		return Uma_RepairModel_Plugin.cancel(context, "The user has cancelled Modelrepair.");
	}
